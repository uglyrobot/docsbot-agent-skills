...
5. Update appearance, `labels.firstMessage`, starter questions, support link, agent settings, source tag vocabulary, actions, lead collection, MCP servers, and skill settings. Never edit the existing `labels.inputPlaceholder` input message label. Keep `showCopyButton` and `linkSafetyEnabled` off/omitted unless the user explicitly confirms the narrowly allowed internal output-sharing use case in [workflow.md](references/workflow.md).
...
- `labels.inputPlaceholder` matches its pre-write value; it was not authored, changed, or reset during setup.
