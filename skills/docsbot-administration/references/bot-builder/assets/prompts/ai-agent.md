...
Exact copy of `PRESET_PROMPTS.AI_AGENT.prompt` from `src/constants/prompts.constants.js`.

Use as the base template for internal knowledge assistants, department assistants, research bots, and general private copilots. Replace placeholders, make minimal in-place additions, and preserve `search_documentation` and other canonical tool names from the source prompt.
